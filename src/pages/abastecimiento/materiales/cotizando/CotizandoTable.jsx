...
import { uidsToRemoveMaterialCotizando } from "../../../../components/utils";
import ModalComponent from "../../../../components/Modal";

const CotizandoTable = ({ data, filter, setFilter, resetState }) => {
  const [open, setOpen] = useState(false);

  const allColumns = useMemo(() => getColumns(data.data[0]), [data.data[0]]);

  const columns = allColumns.filter(
    (item) => !uidsToRemoveMaterialCotizando.includes(item.uid)
  );
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            Total {data.total_count} Solicitudes en cotización
          </span>
